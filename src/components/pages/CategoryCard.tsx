...
const CategoryCard: React.FC<IProps> = ({ title, image, link }) => {
  return (
    <div className="children:transition-all children:duration-300">
      <Link
        href={link}
        className="border-dark-gray group relative flex h-[26rem] origin-top flex-col items-center border border-opacity-20 bg-primary p-10 hover:z-[1] hover:scale-y-105 hover:shadow-2xl"
      >
        <div className="bg-dark-gray absolute left-1/2 top-0 h-1 w-52 origin-top -translate-x-1/2 scale-y-0 group-hover:scale-y-100" />
        <OptimizedImage
          {...image}
        ...
        <div className="absolute bottom-0 left-1/2 flex w-52 origin-bottom -translate-x-1/2 scale-y-0 flex-col group-hover:scale-y-100">
          <button className="border-dark-gray border border-b-0 border-opacity-20 bg-primary p-4">
            <span className="text-2xl font-medium">Browse</span>
          </button>
          <div className="bg-dark-gray h-1" />
        </div>
      </Link>
    </div>
  );
